...
def convert_routes_to_labels(routes):
    return [[f'Node{node}' for node in route] for route in routes]
...
def update(frame):
    iteration = frame // max(len(route) for route in iterations[frame % 20])  # Determine iteration number
    frame_in_iteration = frame % max(len(route) for route in iterations[frame % 20])
    ...
    routes = convert_routes_to_labels(iterations[iteration])
...
frames = sum(len(route) for routes in iterations.values() for route in routes)
